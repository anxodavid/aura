...
export default function SettingsModal({ cliches, onSave, onClose }) {
  ...
  const handleSave = () => {
    const arr = inputStr.split(',').map(s => s.trim()).filter(s => s.length > 0);
    onSave(arr);
  };
  ...
  return (
    <div style={{
      ...
      <div className="glass-panel" style={{ width: '100%', maxWidth: '500px', animation: 'fadeIn 0.2s ease-out' }}>
        <h3 style={{ marginBottom: '16px' }}>Configuración de Clichés</h3>
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '16px' }}>
          Añade frases exactas separadas por comas. Estas frases penalizarán el texto si se detectan.
        </p>
        <textarea
          value={inputStr}
          ...
          style={{
            width: '100%', height: '120px', backgroundColor: 'rgba(0,0,0,0.2)',
            border: '1px solid var(--surface-border)', borderRadius: '8px', padding: '12px',
